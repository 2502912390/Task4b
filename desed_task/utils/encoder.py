...
class ManyHotEncoder:
    ...
    def __init__(
        ...
        n_frames = self.audio_len * self.fs#160000
    ...
    #将帧转成时间[0 1 2 ...]->[0 0.064 0.128 ...]
    def _frame_to_time(self, frame):#（0-157）
    ...
    def encode_strong_df(self, label_df):
        ...
        if type(label_df) is str:
            if label_df == "empty":
                ...
                return y
        y = np.zeros((samples_len, len(self.labels)))#（156 27）
            ...
            if {"onset", "offset", "event_label"}.issubset(label_df.columns):
                for _, row in label_df.iterrows():
                    if not pd.isna(row["event_label"]):
                        i = self.labels.index(row["event_label"])#当前标签对应的idx
                        ...
                        if "confidence" in label_df.columns:
                            y[onset:offset, i] = row["confidence"]  # 对应帧内的值换成置信度
                        else:
                            y[onset:offset, i] = (
    ...
    def state_dict(self):
        return {
            "labels": self.labels,#27
            "audio_len": self.audio_len,#10
            "frame_len": self.frame_len,#2048
            "frame_hop": self.frame_hop,#256
            "net_pooling": self.net_pooling,#4
            "fs": self.fs,#16000
        }
