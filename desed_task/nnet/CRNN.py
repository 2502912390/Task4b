...
class CRNN(nn.Module):
    ...
    def _get_logits_one_head(
        ...
        strong = dense(x)  # [bs, frames, nclass] (60 156 384)->(60 156 27)
        strong = self.sigmoid(strong)
        if classes_mask is not None:
            classes_mask = ~classes_mask[:, None].expand_as(strong)
        if self.attention in [True, "legacy"]: # 改进sof权重
            sof = dense_softmax(x)  # [bs, frames, nclass] (60 156 384)->(60 156 27)
            ...
            weak = (strong * sof).sum(1) / sof.sum(1)  # [bs, nclass] (60 27)
        else:
            weak = strong.mean(1)
        ...
        return strong.transpose(1, 2), weak #(60 27 156) (60 27)
    ...
    def forward(self, x, pad_mask=None, embeddings=None, classes_mask=None):
        ...
        x = self.apply_specaugment(x)# (1 1 128 626)
        x = x.permute(0, 1, 3, 2)
        ...
        # conv features
        x = self.cnn(x)#（60 128 156 1） 频域信息没了？
        ...
        if freq != 1:
            ...
            x = x.squeeze(-1)
            x = x.permute(0, 2, 1)  # [bs, frames, chan] （60 156 128）
        ...
        if self.use_embeddings:
            if self.aggregation_type == "global":
                ...
                reshape_emb = torch.nn.functional.adaptive_avg_pool1d(#（60 156 768） （bs 250 765）要和卷积的输出对其
        ...
        if self.use_embeddings:
            if self.dropstep_recurrent and self.training:
                ...
                x = dropstep(x.transpose(1, -1)).transpose(1, -1)#（60 156 128）
                reshape_emb = dropstep(reshape_emb.transpose(1, -1)).transpose(1, -1)# （60 156 768）
            x = self.cat_tf(self.dropout(torch.cat((x, reshape_emb), -1)))# （60 156 128）
        else:
            if self.dropstep_recurrent and self.training:
                dropstep = torchaudio.transforms.TimeMasking(
                    self.dropstep_recurrent_len, True, self.dropstep_recurrent
        ...
        x = self.rnn(x)
        x = self.dropout(x)#（60 156 384）

        return self._get_logits(x, pad_mask, classes_mask)#（60 27 156） （60 27）
...
#
#     input = torch.randn(3, 128, 626)
#     # input=input.to('cuda:0')
#     emb = torch.randn(3, 768, 496)
#     # emb=emb.to('cuda:0')
#
#     crnn = CRNN(**configs["net"])
#     # crnn.to('cuda:0')
#     strong, weak = crnn(input, embeddings=emb)
#     # print(f'myformers device: {next(self.myformers.parameters()).device}')
#
#     print(input.shape)
#     print(strong.shape)
#     print(weak.shape)
