...
class CRNN(nn.Module):
    ...
    def forward(self, x, pretrain_x, pad_mask=None, embeddings=None):#处理10s的数据
        x = x.transpose(1, 2).unsqueeze(1)#(1 128 626)->(1 1 626 128)
        # conv featuress
        x = self.cnn(x)#(1 128 156 1) (bs ch time_seq freq)
        x = x.squeeze(-1).permute(0, 2, 1)  # (1 156 128) (bs time_seq ch)

        embeddings = self.atst_frame(pretrain_x)#(1 768 250) (bs emb ch)

        if embeddings.shape[-1] != x.shape[1]:
            embeddings = torch.nn.functional.adaptive_avg_pool1d(embeddings, x.shape[1]).transpose(1, 2) #(1 768 250) (1 768 156) (1 156 768)
        else:
            embeddings = embeddings.transpose(1, 2)
        x = self.cat_tf(torch.cat((x, embeddings), -1))# (1 156 128)

        # x = self.rnn(x)#(1 156 128)->(1 156 256)
        # x = self.dropout(x)
        # strong = self.dense(x) #(1 156 256)->(1,156,10) # [bs, frames, nclass]
